...
def read_text_file_as_dicts(input_file_path, delimiter='|'):
    ...
    with open(input_file_path, mode='r') as file:
        # Use DictReader, which treats the first row as headers and creates dictionaries for each row
        reader = csv.DictReader(file, delimiter=delimiter)
        # Append each row as a dictionary to `data`
        for row in reader:
            data.append(row)
...
def process_C(row):
    if row['signal'] == '-' and not IS_DELETION:
        return ""
...
def convert_dynamic_graph(input, output):
    ...
    df =read_text_file_as_dicts(input)

    def parallel_process(rows):
        output=""
        for row in rows:
            output+=process_dynamic(row)
        return output

    def process_dynamic(row):
        process_functions = {
            'C': process_C,     #Comment
    ...
    results = Parallel(n_jobs=20)(delayed(parallel_process)(df[chunks[i]:chunks[i+1]]) for i in range(len(chunks)-1))
...
def convert_static_graph(input, output):
    ...
    with open(output, 'w') as file:
        tmp_string=""
        for node in nodes:
            tmp_string+=f"v {node} 0\n"
            
        lines = tmp_string.strip().split('\n')

        # Convert each line to a list of parts
        data = [line.split() for line in lines]

        # Sort data by the second column (converted to an integer for sorting)
        sorted_data = sorted(data, key=lambda x: int(x[1]))

        # Join the sorted data back into the desired string format
