...
then
    for q in 1 2 3 4
    do
        bash "${SCRIPT_PATH}/${SCRIPT_NAME}" "${q}" "${2}"        
    done
    exit 0
else
    if [[ " ${queries[@]} " =~ " $1 " ]]; 
    then
        ...
        then
            ...
            then
                ...
                SNB_DATA_PATH="${PARENT_SCRIPT_PATH}/data_tool/ldbc_snb/snb/snb_${SCALE_FACTOR}"

                #<PERSON>
                if grep -q '<person>' "${FILE_QUERY_PATH}"; then
                    PERSON_FILE="${SNB_DATA_PATH}/person"
                    if [ -f "${PERSON_FILE}" ]
                    then 
                ...
                if grep -q '<message>' "${FILE_QUERY_PATH}"; then
                    MESSAGE_FILE="${SNB_DATA_PATH}/message"
                    if [ -f "${MESSAGE_FILE}" ]
                    then 
                ...
                if grep -q '<message_tag>' "${FILE_QUERY_PATH}"; then
                    MESSAGE_TAG="${SNB_DATA_PATH}/message_tag"
                    if [ -f "${MESSAGE_TAG}" ]
                    then 
                ...
                if grep -q '<knows>' "${FILE_QUERY_PATH}"; then
                    KNOWS_FILE="${SNB_DATA_PATH}/knows"
                    if [ -f "${KNOWS_FILE}" ]
                    then 
                ...
                if grep -q '<knows1>' "${FILE_QUERY_PATH}"; then
                    KNOWS1_FILE="${SNB_DATA_PATH}/knows1"
                    if [ -f "${KNOWS1_FILE}" ]
                    then 
                ...
                if grep -q '<knows2>' "${FILE_QUERY_PATH}"; then
                    KNOWS2_FILE="${SNB_DATA_PATH}/knows2"
                    if [ -f "${KNOWS2_FILE}" ]
                    then 
                ...
                bash "${SCRIPT_PATH}/exe_query.sh" "${FILE_QUERY_PATH}"
            else 
