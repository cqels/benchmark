...
object Rsnb_q1_10 {
  ...
  def execute(args: Array[String], f: List[Any] => Unit) = 
    bench(args, (dataset: String, parallelMode: Int, timeout: Long, batchSize: Int) => run[Rsnb_q1_10](
      Seq(
        ...
        (new java.io.FileInputStream("/root/benchmark/dbtoaster/dbtoaster_data/snb_10/ouput/dbtoaster.knows.window.csv"),new Adaptor.CSV("KNOWS","string,string,string,string,string","\\Q|\\E", "insert"),Split())
      ), 
...
class Rsnb_q1_10 extends Rsnb_q1_10Base with Actor {
